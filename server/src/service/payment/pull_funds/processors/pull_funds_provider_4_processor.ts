...
const logger = init_logger('pull_funds_provider_4_processor');

export type PullFundsProvider4ProcessorArgs = {
  payment: Payment;
...
/**
 * pull_funds_provider_4_processor
 * --------------------------------
 * Executes a card payment through Provider 4.
 * Fallback processor for when other providers fail.
 *
 * The response is configurable via ProcessorConfig for testing:
 *   ProcessorConfig.set_provider_4_response(PaymentStatusCode.SUCCESS);
 *   ProcessorConfig.set_provider_4_response(PaymentStatusCode.FRAUDULENT);
 *
 * @param args - Payment, payment method, and optional user
 * @returns Result<PaymentStatusCode, ApplicationError>
 */
export default async function pull_funds_provider_4_processor({
  payment,
  payment_method,
  user,
}: PullFundsProvider4ProcessorArgs): Promise<Result<PaymentStatusCode, ApplicationError>> {
  try {
    const config = ProcessorConfig.get_provider_4_response();
    ...
    if (payment_method.type !== PaymentMethodType.CARD) {
      logger.error(
        ...
        `Invalid payment method type for Provider 4`
      );
    ...
    if (payment_method.status !== PaymentMethodStatus.VALID) {
      logger.error(
        ...
        `Invalid payment method status for Provider 4`
      );
      return Result.success(PaymentStatusCode.CARD_INVALID);
    }
    ...
    logger.info(
      ...
      `Provider 4 processor completed with status: ${config.status_code}`
    );
    ...
    logger.error(
      ...
      `Provider 4 processor encountered error`
    );
