...
const logger = init_logger('pull_funds_provider_2_processor');

export type PullFundsProvider2ProcessorArgs = {
  payment: Payment;
...
/**
 * pull_funds_provider_2_processor
 * --------------------------------
 * Executes a card payment through Provider 2.
 * Optimized for higher-value transactions.
 *
 * The response is configurable via ProcessorConfig for testing:
 *   ProcessorConfig.set_provider_2_response(PaymentStatusCode.SUCCESS);
 *   ProcessorConfig.set_provider_2_response(PaymentStatusCode.NSF, 500);
 *
 * @param args - Payment, payment method, and optional user
 * @returns Result<PaymentStatusCode, ApplicationError>
 */
export default async function pull_funds_provider_2_processor({
  payment,
  payment_method,
  user,
}: PullFundsProvider2ProcessorArgs): Promise<Result<PaymentStatusCode, ApplicationError>> {
  try {
    const config = ProcessorConfig.get_provider_2_response();
    ...
    if (payment_method.type !== PaymentMethodType.CARD) {
      logger.error(
        ...
        `Invalid payment method type for Provider 2`
      );
    ...
    if (payment_method.status !== PaymentMethodStatus.VALID) {
      logger.error(
        ...
        `Invalid payment method status for Provider 2`
      );
      return Result.success(PaymentStatusCode.CARD_INVALID);
    }
    ...
    logger.info(
      ...
      `Provider 2 processor completed with status: ${config.status_code}`
    );
    ...
    logger.error(
      ...
      `Provider 2 processor encountered error`
    );
