...
const logger = init_logger('create_provider_2_payment_method_processor');

export type CreateProvider2PaymentMethodProcessorArgs = {
  user: User;
  card_data: CardData;
 ...
 * ------------------------------------------
 * Creates a payment method processor entry for Provider 2.
 *
 ...
 */
export default async function create_provider_2_payment_method_processor({
  user,
  card_data,
  payment_method,
}: CreateProvider2PaymentMethodProcessorArgs): Promise<Result<PaymentMethodProcessor, ApplicationError>> {
  try {
    const config = ProcessorCreationConfig.get_provider_2_config();
    ...
    logger.info(
      ...
      `Starting Provider 2 processor creation`
    );
    ...
    if (!config.should_succeed) {
      logger.warn(
        ...
        `Provider 2 processor creation configured to fail`
      );
      return Result.fail(new ServerInternalError(new Error('Provider 2 creation failed')));
    }

    const processor_result = await PaymentMethodProcessor.create({
      payment_method_id: payment_method.id,
      provider_name: PaymentProvider.PROVIDER_2,
    ...
    if (!processor_result.ok) {
      logger.error(
        ...
        `Failed to create Provider 2 processor record`
      );
    ...
    logger.info(
      ...
      `Provider 2 processor created successfully`
    );
    ...
    logger.error(
      ...
      `Provider 2 processor creation encountered error`
    );
