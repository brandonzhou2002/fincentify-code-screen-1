...
const logger = init_logger('create_provider_3_payment_method_processor');

export type CreateProvider3PaymentMethodProcessorArgs = {
  user: User;
  card_data: CardData;
 ...
 * ------------------------------------------
 * Creates a payment method processor entry for Provider 3.
 *
 ...
 */
export default async function create_provider_3_payment_method_processor({
  user,
  card_data,
  payment_method,
}: CreateProvider3PaymentMethodProcessorArgs): Promise<Result<PaymentMethodProcessor, ApplicationError>> {
  try {
    const config = ProcessorCreationConfig.get_provider_3_config();
    ...
    logger.info(
      ...
      `Starting Provider 3 processor creation`
    );
    ...
    if (!config.should_succeed) {
      logger.warn(
        ...
        `Provider 3 processor creation configured to fail`
      );
      return Result.fail(new ServerInternalError(new Error('Provider 3 creation failed')));
    }

    const processor_result = await PaymentMethodProcessor.create({
      payment_method_id: payment_method.id,
      provider_name: PaymentProvider.PROVIDER_3,
    ...
    if (!processor_result.ok) {
      logger.error(
        ...
        `Failed to create Provider 3 processor record`
      );
    ...
    logger.info(
      ...
      `Provider 3 processor created successfully`
    );
    ...
    logger.error(
      ...
      `Provider 3 processor creation encountered error`
    );
