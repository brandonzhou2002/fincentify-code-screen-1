...
const logger = init_logger('create_provider_4_payment_method_processor');

export type CreateProvider4PaymentMethodProcessorArgs = {
  user: User;
  card_data: CardData;
 ...
 * ------------------------------------------
 * Creates a payment method processor entry for Provider 4.
 *
 ...
 */
export default async function create_provider_4_payment_method_processor({
  user,
  card_data,
  payment_method,
}: CreateProvider4PaymentMethodProcessorArgs): Promise<Result<PaymentMethodProcessor, ApplicationError>> {
  try {
    const config = ProcessorCreationConfig.get_provider_4_config();
    ...
    logger.info(
      ...
      `Starting Provider 4 processor creation`
    );
    ...
    if (!config.should_succeed) {
      logger.warn(
        ...
        `Provider 4 processor creation configured to fail`
      );
      return Result.fail(new ServerInternalError(new Error('Provider 4 creation failed')));
    }

    const processor_result = await PaymentMethodProcessor.create({
      payment_method_id: payment_method.id,
      provider_name: PaymentProvider.PROVIDER_4,
    ...
    if (!processor_result.ok) {
      logger.error(
        ...
        `Failed to create Provider 4 processor record`
      );
    ...
    logger.info(
      ...
      `Provider 4 processor created successfully`
    );
    ...
    logger.error(
      ...
      `Provider 4 processor creation encountered error`
    );
